...
import { gql } from 'graphql-tag'  // Import the GraphQL tag 
...
const NewsPage: React.FC = () => {
  const { data, loading, error } = useQuery<{ articles: Article[] }>(ARTICLES_QUERY)
  ...
  return (
    <div className="container">
      <header>
        ...
        <p>All the latest Web3&Crypto news</p>
        <span>
          ...
          <p><a href="https://github.com/zenvar/puppeteer">A Demo By @Zenvar</a>  </p>
        </span>
      </header>
      ...
      <div className="timeline">
        {data?.articles.map((article) => (
          <div key={article.id} className="timeline-item">
            <div className="date">{article.time}</div>
            <div className="content">
              <h2>{article.header}</h2>
              <p>{article.content}</p>
              <Link to={routes.newsDetails({ id: article.id })}>Read more</Link>
            </div>
          </div>
        ))}
      </div>
